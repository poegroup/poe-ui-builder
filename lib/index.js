...
exports.scripts = function(path, ignore, out, opts, fn) {
  ...
  resolve(path, opts, function(err, tree) {
    ...
    if (opts.poe) poe.init(tree);

    Build.scripts(tree)
      .use('scripts', poe.js(opts.poe), plugins.js())
...
exports.styles = function(path, ignore, out, opts, fn) {
  ...
  resolve(path, opts, function(err, tree) {
    ...
    if (opts.poe) poe.init(tree, true);
    ...
    Build.styles(tree)
      ...
      .end(function(err, str) {
        ...
        mkdirp(dirname(out), function(err) {
          if (err) return fn(err);
          write(out, str, 'utf8', done);
        });
      });

    if (opts.ignoreFiles) return;
    Build.files(tree, {destination: dirname(out)})
